			...
			onFinish: res => {
				...
				switch (res.kind) {
					case 'SUCCESS':
						setTitle('Success!');
						setDescription(
							`Successfully installed ${res.manifest.name}.`,
						);
						break;
					case 'ALREADY_INSTALLED':
						setTitle('Already installed');
						setDescription(
							`${res.manifest.name} is already installed.`,
						);
						break;
					case 'FAILED':
					...
					case 'UNREACHABLE':
						...
						setDescription(
							<>
								Please make sure Discord is open with the latest
								version of Replugged installed and try again.
								<br />
								<div className={styles.buttons}>
									...
									<a
										onClick={() => makeRequest()}
										className={sharedStyle['button-link']}
									>
										<span>Try again</span>
									</a>
								</div>
