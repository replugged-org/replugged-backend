...
export default function Tooltip({ children, text, position, align, disabled }: TooltipProps) {
  ...
  if (display && elementRef.current) {
    ...
    if (position === "bottom") {
      ...
      css.top =
        rect.y +
        rect.height +
        (tooltipElement ? tooltipElement.getBoundingClientRect().height : 32) +
        6;
    }

    tooltip = createPortal(
      <div ref={tooltipRef} className={className.join(" ")} style={css}>
        {text}
      </div>,
      document.body,
    );
  ...
  return h(
    Fragment,
    null,
    cloneElement(children, { ref: elementRef, onMouseEnter, onMouseLeave }),
    tooltip,
  );
}
