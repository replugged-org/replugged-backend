...
function ProfileBadges({ flags }) {
    return (<div className={style.badges} style={{ color: '#7289da' }}>
            {Boolean(flags & UserFlags.DEVELOPER) && (<Tooltip text='Replugged Developer' align='center'>
                    {/* @ts-ignore */}
                    <Developer className={style.badge}/>
                </Tooltip>)}
            {Boolean(flags & UserFlags.STAFF) && (<Tooltip text='Replugged Staff' align='center'>
                    {/* @ts-ignore */}
                    <Staff className={style.badge}/>
                </Tooltip>)}
            {Boolean(flags & UserFlags.SUPPORT) && (<Tooltip text='Replugged Support' align='center'>
                    {/* @ts-ignore */}
                    <Support className={style.badge}/>
                </Tooltip>)}
            {Boolean(flags & UserFlags.CONTRIBUTOR) && (<Tooltip text='Replugged Contributor' align='center'>
                    {/* @ts-ignore */}
                    <Contributor className={style.badge}/>
                </Tooltip>)}
            {Boolean(flags & UserFlags.TRANSLATOR) && (<Tooltip text='Replugged Translator' align='center'>
                    {/* @ts-ignore */}
                    <Translator className={style.badge}/>
                </Tooltip>)}
            {Boolean(flags & UserFlags.BUG_HUNTER) && (<Tooltip text='Replugged Bug Hunter' align='center'>
                    {/* @ts-ignore */}
                    <Hunter className={style.badge}/>
                </Tooltip>)}
            {Boolean(flags & UserFlags.EARLY_USER) && (<Tooltip text='Replugged Early User' align='center'>
                    {/* @ts-ignore */}
