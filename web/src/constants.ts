export const Endpoints = {
	...
	DOCS_CATEGORIZED: (cat: string, doc: string) =>
		`/api/v1/docs/${cat}/${doc}`,

	STORE_ITEMS: (type: string) => `/api/v1/store/items/${type}`,
	STORE_ITEM: (type: string, id: string) =>
		`/api/v1/store/items/${type}/${id}`,
	...
	BACKOFFICE_FORMS:
		'/api/v1/backoffice/forms?kind=hosting&kind=publish&kind=verification',
	...
	BACKOFFICE_GET_USERS_GUILD_PERKS: (user: string) =>
		`/api/v1/backoffice/users/perks/guild/${user}`,
...
export const Routes = {
	...
	STORE_SUGGESTIONS:
		'https://github.com/replugged-org/suggestions/issues?q=is%3Aissue+is%3Aopen+label%3A%22up+for+grabs%22',
	...
	BACKOFFICE_STORE_FORMS_FORM: (id: string) =>
		`/backoffice/store/forms/${id}`,
	BACKOFFICE_STORE_REPORTS: '/backoffice/store/reports',
	BACKOFFICE_STORE_REPORTS_REPORT: (id: string) =>
		`/backoffice/store/reports/${id}`,
