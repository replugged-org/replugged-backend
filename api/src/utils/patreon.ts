...
export async function fetchPledgeStatus(
	token: OAuthToken,
): Promise<[boolean, CutieStatus]> {
	const query = new URLSearchParams();
	query.set('include', 'memberships');
	query.set(
		'fields[member]',
		'patron_status,currently_entitled_amount_cents,next_charge_date',
	);
	...
	data.pledgeTier = DONATION_TIERS.findIndex(
		v => v > pledgeData.currently_entitled_amount_cents,
	);
	data.perksExpireAt =
		new Date(pledgeData.next_charge_date).getTime() + GRACE_PERIOD;
	return [donated, data];
}

export async function prepareUpdateData(
	patreonAccount: OAuthToken,
): Promise<UpdateFilter<User>> {
	let or = new Long(0);
...
export async function updateDonatorState(
	...
	const statusChange =
		user.cutieStatus?.pledgeTier !==
		mongoUpdate.$set!['cutieStatus.pledgeTier'];
	if (manual) {
		// @ts-ignore
...
export async function refreshDonatorState(
	mongo: MongoClient,
	user: User,
	manual?: boolean,
) {
	...
	if (
		!manual &&
		(perksExpireAt > Date.now() || user.flags & UserFlags.CUTIE_OVERRIDE)
	) {
		return;
	}
