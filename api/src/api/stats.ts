...
function datesOverTime (dates: number[], pointsCount: number = 50): GraphOverTime {
  ...
  const data: GraphOverTime = { count: dates.length,
    allTime: [ dates.length ],
    month: [ dates.length ],
    week: [ dates.length ] };
  const buffer: { count: number, allTime: number, month: number, week: number } = { count: dates.length,
    allTime: 0,
    month: 0,
    week: 0 };
...
function formatPeriodicData (data: PeriodicData[], pointsCount = 50): GraphPeriodic | null {
  ...
  const graph: GraphPeriodic = { month: [],
    week: [],
    day: [] };
