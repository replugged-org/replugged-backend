...
function sendUser(
  ...
  const etag = `W/"${createHash("sha256")
    ...
    .update(user._id)
    .update((user.updatedAt ?? DATE_ZERO).toISOString())
    .digest("base64url")}"`;
...
async function patchSelf(
  ...
  if (!request.body || typeof request.body !== "object") {
    reply.code(400).send({ error: 400, message: "Invalid request body." });
    return;
  }

  if (
    "cutiePerks" in request.body &&
    request.body.cutiePerks &&
    typeof request.body.cutiePerks === "object"
  ) {
  ...
  const result = await this.mongo
    .db!.collection<DatabaseUser>("users")
    .findOneAndUpdate({ _id: request.user!._id }, { $set: update }, { returnDocument: "after" });
...
async function refreshPledge(
  ...
  if (!patreonAccount) {
    reply
      .code(422)
      .send({ error: 422, message: "This operation requires a linked Patreon account" });
    return;
  }
