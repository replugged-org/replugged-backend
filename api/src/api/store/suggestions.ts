...
function gqlQuery(after: string): string {
  return `query {
    repository(name: "suggestions", owner: "replugged-org") {
      issues(states: OPEN, first: 100, labels: ["up for grabs", "work in progress"], orderBy: {field: CREATED_AT, direction: DESC}${
        after ? `, after: "${after}"` : ""
      }) {
...
export async function fetchSuggestions(): Promise<Suggestion[]> {
  return getOrCompute(
    ...
    async () => {
      ...
      return all
        .sort((a, b) =>
          a.reactions.totalCount > b.reactions.totalCount
            ? -1
            : a.reactions.totalCount < b.reactions.totalCount
            ? 1
            : 0,
