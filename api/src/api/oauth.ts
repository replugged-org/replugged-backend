...
async function authorize(this: FastifyInstance, request: FastifyRequest<AuthorizationRequestProps>, reply: Reply) {
    ...
    reply.redirect(
        getAuthorizationUrl(reply.context.config.platform, redirect, reply.context.config.scopes, state)
    )
}
    ...
    if (reply.context.config.platform === "discord") {
        ...
        const res = await collection.findOneAndUpdate(
            { _id: account.id },
            {
                ...
                $min: { createdAt: date },
                $bit: { flags: { and: new Long(((1n << 64n) - 1n) & ~BigInt(UserFlags.GHOST), true) } },
                $set: {
